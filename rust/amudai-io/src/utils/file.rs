//! Implementations of common I/O traits for `File` objects.
...
use crate::{ReadAt, SealingWrite, StorageProfile, WriteAt, verify};
...
impl ReadAt for File {
    ...
    fn read_at(&self, range: Range<u64>) -> std::io::Result<Bytes> {
        ...
        file_read_at_exact(self, range.start, &mut buf)?;
        Ok(Bytes::from(buf))
    }
