...
use crate::write::stripe_builder::{PreparedStripe, StripeBuilder, StripeBuilderParams};
...
use amudai_sequence::json_printer::FrameToJson;

#[test]
fn test_prepared_fields_decoding() -> Result<()> {
    let record_count = 20000;
    let prepared_stripe = make_prepared_stripe(record_count)?;
...
#[test]
fn test_prepared_stripe_frame_reader() -> Result<()> {
    let record_count = 20000;
    let prepared_stripe = make_prepared_stripe(record_count)?;

    let frame_reader = prepared_stripe.create_frame_reader(None, None)?;
    for frame in frame_reader {
        let frame = frame?;
        let s = frame.to_json(None).unwrap().to_string();
        assert!(s.contains("\"Prop1\":"));
    }
    Ok(())
}

fn make_prepared_stripe(record_count: usize) -> Result<PreparedStripe> {
